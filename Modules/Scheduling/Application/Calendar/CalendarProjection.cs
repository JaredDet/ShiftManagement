...
{
    public static IQueryable<CalendarResponse> ToCalendarResponse(this DbContext context)
    {
        var shifts = context.Set<Shift>().AsNoTracking();
        var assignments = context.Set<ShiftAssignment>().AsNoTracking();
        ...
        var query =
            ...
            join p in positions on s.PositionId equals p.Id

            join a in assignments on s.Id equals a.ShiftId into ag
